...
- **`/servers/self` via the raw-requests hatch.** The canopy client generates its per-endpoint methods from canopy's OpenAPI fetched *live* at build time, so a generated `servers_self()` only appears once canopy PR #396 is deployed — coupling our compile to canopy's deploy. Instead the healer calls the endpoint by literal path through the `raw-requests` escape hatch, which `endpoint_url` still routes `/public/servers/self` over tailscale (device resolved from tailnet identity) and `/servers/self` over mTLS. This compiles today and simply returns `404` → `Deferred` until canopy ships the endpoint. The small response type is defined locally.
