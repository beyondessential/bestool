...
- **Locks never sit on a segment's own bytes.** Windows byte-range locks are enforced rather than advisory, and both modes are unusable over a segment: exclusive stops every reader of a live segment, and shared stops even the writer holding it from appending. A segment's lock is therefore taken on an empty `<segment>.lock` file beside it, the same shape the directory lock already had. Nothing reads or writes those files for content, so the lock answers the only question asked of it — is a session writing this segment? — without standing in anyone's way, and it behaves the same on every platform. The writer deletes its own lock file as the segment closes; compaction sweeps ones left by a session that crashed.
