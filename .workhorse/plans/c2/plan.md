...
- **Defaults.** Startup recall budget 4 MiB of query text; per-record recall cutoff 10 KiB; unwritable-store backlog 1000 records and 16 MiB; retention 12 months.
...
- **File naming.** Segments `audit-<instance-uuid>.jsonl`; period files `audit-<YYYY>-<MM>.jsonl.zst`. Legacy files are `audit-main.redb`, `audit-working-*.redb`, `audit-orphaned-*.redb`.
...
- **Locks.** Advisory lock on the segment file held by its writer for the session; advisory lock on a directory-level lock file held by compaction. Writers never take the directory lock. Use a small cross-platform crate rather than raw fcntl/LockFileEx.
...
- **Startup history read order.** Open segments and period files newest-first by file, and within a file read from the end where the format allows, stopping when the budget is met. Segment count therefore barely affects startup.
- **Compaction throttle.** Run only when at least one closed segment from a previous month exists. Process at most one month per session start. Lowest IO and thread priority the platform offers.
- **Legacy import** needs redb to read the old files, so the dependency stays for the reader only. Group by the old `instance_id` where present; otherwise one import segment.
...
- [ ] Segment writer: create the segment on session start, take its advisory lock, write the opening context record, append query records, append the end record on clean exit.
- [ ] Hash chain: compute `prev` from the previous line's raw bytes; empty for the first record.
...
- [ ] Period file parser: zstd-compressed JSON lines.
- [ ] Merged reader: time-ordered k-way merge across segments and period files, context carried forward, dedup by (instance, seq), time-range filter, newest/oldest limit, streaming with a bounded window.
- [ ] Chain verification per segment and per segment-within-period-file, reporting the first break; chain heads per segment.
...
- [ ] Eligibility: closed segments (lock acquirable) whose start month has ended.
- [ ] Compaction: write period file to a temporary name, sync, rename, then delete consumed segments; directory lock held throughout, skip if held.
- [ ] Retention: delete period files whose month ended longer ago than the retention period; period configurable to longer only.
