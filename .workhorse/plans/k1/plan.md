...
The certificate reading is the larger of `HttpRuntime`'s two: the admin config's
active subjects, the on-disk store reduced to the live cert per set of names,
the manually-loaded certs, and a handshake per concrete name all sit behind
`certificates`. What is left in the check is the expiry bands and the
served-against-configured comparison.

Three things the move settled. **Nothing served to compare is not a mismatch** —
a wildcard names no host to dial, and a handshake that did not answer said
nothing, so `served_leaf` is `Option` and the check grades only `Some(false)`.
**The substrate dedups**, because the same certificate is reachable through
several subjects and sources and a substrate reports each one once. **The check
keeps the name `caddy_certs`**, which is wire-visible and keyed on by canopy's
severity map; renaming it is a change to make deliberately, not in passing.
...
- [x] Add `certificates` to `HttpRuntime`, and read `caddy_certs` through it
