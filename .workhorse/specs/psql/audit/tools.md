...
The API opens an audit directory and reads it as one time-ordered stream of records across all segments and period files, with each query record's context carried forward so the caller sees flat entries.
...
The API verifies hash chains, reporting per segment whether the chain holds and where it first breaks (see [AUD-STO](store.md)).
It exposes the current chain head of every segment, which is what an off-box witness would publish.
...
The verify command checks every segment and period file and reports any chain that does not hold, exiting non-zero if any fails.
