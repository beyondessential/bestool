...
- [ ] Whether a board with no strong source — no TPM, no burnt OTP, only its platform serials — is shippable at all, or is refused the way non-unique hardware is. This decides how much the derivation cost has to carry, and so it comes before the parameters below rather than after them.
- [ ] argon2id parameters, which want measuring on a Pi 5 against the 4 GB floor. Sized for the weakest source we are willing to ship, which the question above settles. Reference figures on a 12-core x86 desktop, argon2id with a 32-byte output at four lanes: 512 MiB costs 0.13 s at one pass and 0.30 s at three; 1 GiB costs 0.26 s and 0.60 s; 2 GiB costs 0.67 s and 1.37 s. Argon2 is memory-bandwidth bound, so a Pi 5 on LPDDR4X will be several times slower, and the figure that matters is the Pi one.
- [ ] Whether production burns 256 bits of true random into Raspberry Pi customer OTP, which is what puts the Pi path on the same footing as a UEFI machine with a TPM.
