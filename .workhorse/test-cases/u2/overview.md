...
- [ ] Reaching the end of the precedence with no usable source fails and reports, rather than deriving from a placeholder (verifies spec: BLI-BID)
- [ ] A board that gains a stronger source derives a different board ID, and the device reports that its sticker no longer matches rather than advertising a handle nobody can match (verifies spec: BLI-BID)
...
- [ ] The cached secret is used when it corresponds to the board, and the device derives again when the cache is absent or belongs to another board, as when an SD card moves between boards (verifies spec: BLI-KEY)
- [ ] Both the QR payload and the advertisement carry the version marker, and a device asked for a version it does not support says so (verifies spec: BLI-KEY)
...
- [ ] The advertisement and scan response together carry the service UUID, handle, salt, and version within the legacy 31-byte limit of each, with the service UUID in the advertisement where scan filtering sees it (verifies spec: BLI-ADV)
...
- [ ] The same board produces the same sticker every time, so a damaged sticker is replaced by generating it again (verifies spec: BLI-STK)
...
- [ ] The web test page completes scan, match, handshake, and both directions against a real device
