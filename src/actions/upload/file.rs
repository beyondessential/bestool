...
pub struct FileArgs {
	...
	/// File which contains the pre-auth token to use.
	///
...
pub async fn run(mut ctx: Context<UploadArgs, FileArgs>) -> Result<()> {
	let token = if let Some(token) = ctx.args_sub.token.clone() {
		Some(decode_token(&token)?)
	} else if let Some(token_file) = ctx.args_sub.token_file.as_deref() {
		let mut file = File::open(token_file).await.into_diagnostic()?;
	...
	if let Some(token) = token {
		...
		token_upload(ctx.erased(), token, &file).await
