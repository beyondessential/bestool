...
pub async fn run(ctx: CheckContext) -> Check {
	...
	match client.query_one("SELECT version()", &[]).await {
		Ok(row) => match row.try_get::<_, String>(0) {
			Ok(v) => Check::pass("version", v.clone()).with_detail("pg_version", v),
			...
			Err(err) => Check::broken("version", "row decode failed", err.to_string()),
		},
		Err(err) => query_error_check("version", &err),
	}
}
