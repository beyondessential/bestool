//! The alertd daemon: the thing that runs the healthchecks on a schedule.
//!
//! The checks themselves, and the machinery they're built from, live in the
//! `bestool-alertd` crate, so a consumer can harvest checks without pulling in
//! the HTTP server, the backup registry, or the service registration here.
//! This module is the composition: it wires a schedule, an HTTP server, and a
//! canopy client together.
...
impl DaemonConfig {
	...
	pub fn with_metrics_handle(mut self, metrics: doctor::DoctorMetricsHandle) -> Self {
