...
use bestool_alertd::canopy::connect;

use crate::alertd::{
	DaemonConfig, LogError, context::InternalContext, http_server, metrics, tasks::TaskContext,
};
...
///
/// Held by [`TaskContext`](crate::alertd::tasks::TaskContext) so a task that has
...
pub async fn run_with_shutdown(
	...
	crate::alertd::child_confinement::confine_children();
	...
	.await
	{
		Ok(Some(client)) => {
			...
			tokio::spawn(async move {
				let mut interval = tokio::time::interval(bestool_alertd::canopy::CERT_RENEW_AFTER);
