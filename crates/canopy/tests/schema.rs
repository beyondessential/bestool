...
/// Pre-upgrade migration testing, gated on the OpenAPI document actually
/// carrying it: a build against a canopy that predates it skips these.
#[cfg(canopy_migration_testing)]
mod migration_testing {
	use bestool_canopy::schema::{
		MigrationArgs, MigrationTimingArgs, VerificationArgs, WorklistEntry,
	};
